...
The aim of this package is to minimize change in user code while leveraging multiple compute devices.  Construct Theano variables and graphs as usual, and instead build functions through this package rather than directly through Theano.  All parallelism is automated and hidden from view.  Workers waiting for a signal from the master, and they only perform the same function the master is performing and all at the same time.

The only additional algorithmic consideration is communication over Theano shared variables.  A distinct version of each one exists on each GPU, so collective communications, such as broadcast, all-reduce, and scatter, are provided to manage workers' values.  Currently, this must be done manually with Synkhronos, as Theano function updates apply only locally within the worker.
